...
\begin{example}[Gaussian Summation Formula]\label{thm:gaussiansum}
...
\begin{proof}
    There are two cases.
    \begin{case}[Case 1]
        Hypothetically, suppose that $\ell > k$.
        We would then have $k = 2\ell > 2k = k + k \geq k$,
        so that $k > k$, implying $k \neq k$. \contradiction
        This shows us $\ell \leq k$.
    \end{case}
    \begin{case}[Case 2]
        Again, assume towards a contradiction that $\ell > k$.
        This would mean $k = 2\ell + 1 > 2k + 1 > 2k = k + k \geq k$,
        so $k > k$, implying $k \neq k$. \contradiction
        Thus, mirroring the first case, we must have $\ell \leq k$.
    \end{case}
\end{proof}
...
\begin{note}
    ...
    % This is entirely equivalent to assuming $\pn*{\forall \ell \leq k}\pn*{\varphi(\ell)}$
    % and then proving $\varphi(k + 1)$, as written in \autoref{thm:stronginduction}.
\end{note}
