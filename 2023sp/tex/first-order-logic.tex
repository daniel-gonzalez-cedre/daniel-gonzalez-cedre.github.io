...
\begin{center}
    ...
    \begin{minipage}[t]{.45\linewidth}
        \begin{table}[H]
            ...
            \begin{tabular}{|Cc|Cc|r|} \hline
                \multicolumn{3}{|c|}{\thead{Unquantified Inference Rules}} \\\hline
                \thead{Deduction} & \thead{If by assuming \(p\) \\ we are able to derive \(q\),\\ then we can derive \(p \rightarrow q\)} & \thead[r]{\(p \implies q\) \\[1ex] \hline \\[-2ex] \(p \rightarrow q\)} \\ \hline
                \thead{Modus Ponens} & \thead{If we know \(p \rightarrow q\) \\  and we have \(p\),\\ then we can derive \(q\)} & \thead[r]{\(p\) \\ \(p \rightarrow q\) \\[1ex] \hline \\[-2ex] \(q\)} \\ \hline
                \thead{Modus Tollens} & \thead{If we know \(p \rightarrow q\) \\ and we have \(\neg q\),\\ then we can derive \(\neg p\)} & \thead[r]{\(\neg q\) \\ \(p \rightarrow q\) \\[1ex] \hline \\[-2ex] \(\neg p\)} \\ \hline
                \thead{Reductio \\ ad Absurdum} & \thead{If by assuming \(p\) \\ we can derive both \(q\) and \(\neg q\),\\ then we can derive \(\neg p\)} & \thead[r]{\(p \implies q\) \\ \(p \implies \neg q\) \\[1ex] \hline \\[-2ex] \(\neg p\)} \\ \hline
            \end{tabular}
        \end{table}
    \end{minipage}
\end{center}
...
\begin{table}[H]
    ...
    \begin{tabular}{|Cc|Cc|r|} \hline
        ...
        \thead{Universal Instantiation \\ (\(\forall\) Elimination)} & \thead{If we know \(\forall x \pn*{\varphi(x)}\), \\ then we can derive \(\varphi(t)\) for any \(t\).} & \thead[r]{\(\forall x \pn*{\varphi(x)}\) \\[1ex] \hline \\[-2ex] \(\varphi(t)\) for any \(t\)} \\ \hline
        \thead{Universal Generalization \\ (\(\forall\) Introduction)} & \thead{If we know \(\varphi(t)\) for an arbitrary \(t\), \\ then we can derive \(\forall x \pn*{\varphi(x)}\).} & \thead[r]{\(\varphi(t)\) for an arbitrary \(t\) \\[1ex] \hline \\[-2ex] \(\forall x \pn*{\varphi(x)}\)} \\ \hline
        \thead{Existential Instantiation \\ (\(\exists\) Elimination)} & \thead{If we know \(\exists x \pn*{\varphi(x)}\), then we can derive \\ \(\varphi(t)\) for some \(t\) that hasn't yet appeared.} & \thead[r]{\(\exists x \pn*{\varphi(x)}\) \\[1ex] \hline \\[-2ex] \(\varphi(t)\) for some \(t\) that hasn't yet appeared} \\ \hline
        \thead{Existential Generalization \\ (\(\exists\) Introduction)} & \thead{If we know \(\varphi(t)\) for some particular \(t\), \\ then we can derive \(\exists x \pn*{\varphi(x)}\).} & \thead[r]{\(\varphi(t)\) for some particular \(t\) \\[1ex] \hline \\[-2ex] \(\exists x \pn*{\varphi(x)}\)} \\ \hline
    \end{tabular}
\end{table}
