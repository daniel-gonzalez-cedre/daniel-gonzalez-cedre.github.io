...
#definition(name: "Graph")[
  A #define[graph] $G$ is determined by a _finite_ set of #define[vertices] $V(G)$ and a set of #define[edges] $E(G)$ satisfying $E(G) subset {W st W subset V(G) and card(W) = 2}$.
]
...
#theorem(name: "Handshake Lemma (v. 1)")[
  For any graph $G$, we have $display(sum_(v in V(G)) deg(G, v)) = 2 dot card(E(G))$.
]

#theorem(name: "Handshake Lemma (v. 2)")[
  // For any graph $G$, we have $$
  henlo
]

#theorem(name: "Handshake Lemma (v. 3)")[
  henlo
]
